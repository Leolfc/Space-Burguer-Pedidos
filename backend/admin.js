document.addEventListener("DOMContentLoaded", () => {
    ...
    async function carregarLanches() {
        try {
            const response = await fetch(`${API_BASE}/buscar/hamburguers?t=${Date.now()}`, { cache: "no-store" });
            ...
            lanches.forEach((lanche) => {
                ...
                const categoriasTexto = Array.isArray(lanche.categoria) ? lanche.categoria.join(", ") : '';
                const imgSrc = lanche.imagem_url ? (lanche.imagem_url.startsWith("/uploads") || lanche.imagem_url.startsWith("/img") ? `${API_BASE}${lanche.imagem_url}` : lanche.imagem_url) : "";
                const statusHtml = `
                    ${lanche.novoItem ? '<span class="badge badge-novo">Novo</span>' : ""}
                    ${lanche.indisponivel ? '<span class="badge badge-indisponivel">Indisponível</span>' : '<span class="badge">Ativo</span>'}
                `;

                linha.innerHTML = `
                    <td>${imgSrc ? `<img src="${imgSrc}" alt="${lanche.nome}" style="width:48px;height:48px;object-fit:cover;border-radius:6px;"/>` : ""}</td>
                    <td>${lanche.nome}</td>
            ...
            tabelaCorpo.innerHTML = '<tr><td colspan="6">Erro ao carregar cardápio.</td></tr>';
        }
    }
    ...
    function entrarNoModoEdicao(lanche) {
        ...
        document.getElementById("preco").value = String(lanche.preco).replace('.', ',') || ""; // Converte ponto para vírgula
        ...
        formAdicionar.querySelectorAll('input[name="categoria"]').forEach((cb) => {
            cb.checked = Array.isArray(lanche.categoria) && lanche.categoria.includes(cb.value);
        });
    ...
    if (formAdicionar) {
        formAdicionar.addEventListener("submit", async (event) => {
            ...
            const precoValor = document.getElementById("preco").value.replace(',', '.');
            ...
            formData.append("indisponivel", document.getElementById("indisponivel").checked);
            ...
            const url = idEmEdicao ? `${API_BASE}/editar/hamburguer/${idEmEdicao}` : `${API_BASE}/adicionar/hamburguers`;
            const method = idEmEdicao ? 'PUT' : 'POST';

            try {
                const response = await fetch(url, { method, body: formData });
                if (response.ok) {
                    alert(`Lanche ${idEmEdicao ? 'atualizado' : 'adicionado'} com sucesso!`);
                    sairDoModoEdicao();
                    carregarLanches();
                    mostrarTela("tela-gerenciar");
                } else {
                    const erro = await response.json();
                    throw new Error(erro.message || `Erro ao ${idEmEdicao ? 'atualizar' : 'adicionar'} lanche.`);
                }
            } catch (error) {
        ...
        tabelaCorpo.addEventListener("click", async (event) => {
            ...
            if (target.classList.contains("btn-deletar")) {
                if (confirm("Você tem certeza que deseja deletar este lanche?")) {
                    try {
                        const response = await fetch(`${API_BASE}/deletar/hamburguer/${id}`, { method: 'DELETE' });
                        if (response.ok) {
            ...
            if (target.classList.contains("btn-editar")) {
                try {
                    const response = await fetch(`${API_BASE}/buscar/hamburguer/${id}?t=${Date.now()}`);
